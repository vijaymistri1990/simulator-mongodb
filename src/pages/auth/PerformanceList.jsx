...
import { Page, Layout, LegacyCard, IndexTable, Button, useIndexResourceState, Badge, Text } from '@shopify/polaris';
import UserHeader from '../../components/UserHeader';
import {ApiCall, GetUserApiCall, UserApiCall} from "../../helper/axios";
...
function PerformanceList() {
    ...
    const saveResult = async (value) => {
        ...
        if(resultId){
            ...
            let res = await UserApiCall('PUT', '/performance-result', data, header);
            if (res.data.status === 'success' && res.data.statusCode === 200) {
                setResultId(0)
                setActive(!active);
                GetPerformanceData();
